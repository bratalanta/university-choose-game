import { getCurrentUserData, goNextLevel, reduceHeartsCount, reducePoints, startLevel } from "./common.js";
...
for (let i = 0; i < PicturesCount[complexity]; i++) {
    ...
    while (picturePositions.some((pic) => Math.abs(pic - absolutePosition) < SAFE_DISTANCE) && i > 0) {
...
container.addEventListener('click', ({target}) =>
    target.classList.contains('pictures__food') ? target.classList.toggle('chosen') : null)
